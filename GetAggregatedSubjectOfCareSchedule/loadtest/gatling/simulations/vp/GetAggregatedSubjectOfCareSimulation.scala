...
class GetAggregatedSubjectOfCareSimulation extends Simulation {
    ...
    .during(testTimeSecs) { 		
      exec(
        http("GetAggregatedSubjectOfCareSchedule")
          ...
          .fileBody("GetSubjectOfCareSchedule_Positive_Response.xml").asXML
  				.check(status.is(200))
          .check(xpath("soap:Envelope", List("soap" -> "http://schemas.xmlsoap.org/soap/envelope/")).exists)
          .check(xpath("//hdr:ProcessingStatusList", List("hdr" -> "urn:riv:interoperability:headers:1")).count.is(3))
          .check(xpath("//resp:timeslotDetail", List("resp" -> "urn:riv:crm:scheduling:GetSubjectOfCareScheduleResponder:1")).count.is(6)
        )
      )
